...
export async function GET() {
  try {
    ...
    // 查询用户是否有正在处理中的lipsync generation任务
    const { data: pendingGenerations, error: queryError } = await supabase
      .from('lipsync_generations')
      .select('id, job_id, created_at, generation_mode, selected_model, audio_prompt')
      .eq('user_id', user.id)
    ...
    if (queryError) {
      console.error('[LipSync Check Pending API] Query error:', queryError);
    ...
    console.error('[LipSync Check Pending API] Unexpected error:', error);
    return NextResponse.json({
